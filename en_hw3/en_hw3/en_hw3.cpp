...
list<string>bc_driver(list<string>kb, string query)
{
	string theta;
...
int main()
{
	...
	list<string>ans = bc_driver(kb, query);
	list<string>::iterator i;
	//cout << "ANSWER:";
	string answer = "TRUE";
	if (ans.empty())
	...
	/*char ch;
	cin >> ch;*/
	/*string x = "Infected(John,a,b,c,d,e)";
	vector<string>ans = vectorify(get_args(x));
	...
	string goal = "Diagnosis(John,Fever)";
	list<string>kb;
	kb.push_back("Works(x,Aidagency)&HasTraveled(x,Timbuktu)=>Diagnosis(x,Infected)");
	kb.push_back("Diagnosis(x,Fever)=>HasSymptom(John,Fever)");
	kb.push_back("HasSymptom(x,Fever)&HasTraveled(x,Timbuktu)=>Works(Alice,Aidagency)");
	kb.push_back("Works(John,Aidagency)");
	kb.push_back("Diagnosis(John,Fever)");
	kb.push_back("HasTraveled(John,Timbuktu)");
	list<string>ans = fetch_rules(kb, goal);
	list<string>::iterator i;
	for (i = ans.begin(); i != ans.end(); i++)
	{
		cout << *i << endl;
	...
	/*string subs = "x|John";
	string x = "Anorexic(x,Example)";
	cout << sub(subs, x) << endl;
	char ch; cin >> ch;*/
}
